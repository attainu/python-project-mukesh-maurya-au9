    ...
    while True:                                            #Repeat Menu until stops.
        ...
        if (len(input_1) == 1):                                           #Checking input length.
            if (input_1 == 'O'):                                          #If input is "O".
                ...
                break                                                     #Stop repeating Main Menu.
            elif (input_1 == 'X'):                                        #If input is "X".
                print("*" * 30 + " THANK YOU " + "*" * 31 + "\n")           #Good bye comment.
                break                                                     #Stop repeating Main Menu.
            else:                                                                                 #If O, R, P, X not inserted then...
...
def def_order_menu():                                                                               #yousef
    while True:                                             # While looping to keep menu alive
        ...
        input_1 = str(input("What You Want To Choose: ")).upper() # Options Handling : F-O-M-X.
        if len(input_1) == 1:
            if (input_1 == 'F'):  #Easy Access Checking Logic
                ...
                print("*" * 31 + " THANK YOU FOR VISIING ON DELICEUS GO! " + "*" * 30 + "\n")
                break
            else:
...
def def_full_file_reader():                                                                         #mustafa
    file_foods = open('' +navigator_symbol+'/home/mukesh/AttainU/python-project-mukesh-maurya-au9/DeliceusGo/files/list_foods.fsd', 'r') # Reading Food List
    ...
    file_drinks = open(''+navigator_symbol+'/home/mukesh/AttainU/python-project-mukesh-maurya-au9/DeliceusGo/files/list_drinks.fsd', 'r') # Reading Drinks List
    for i in file_drinks:
        list_drinks.append(str(i.strip()))
    file_drinks.close()

    file_services = open(''+navigator_symbol+'/home/mukesh/AttainU/python-project-mukesh-maurya-au9/DeliceusGo/files/list_services.fsd', 'r') # Reading Services
    ...
    i = 0
    while i <= (len(list_drinks) - 1): #Enumarte through food list to filter out prices and setup print Formatting by replacing spaces with count difference of string length and align Prices to the most left of the terminal
...
def def_food_drink_order():
    while True:
            ...
            print("\n (M) MAIN MENU                   (P) PAYMENT                   (x) EXIT\n" + "_" * 74)
            print()

            input_1 = input("Please Select Your Choise: ").upper() #Handling Menu Selection
            if (input_1 == 'M'):
                print("\n" * 10)
                def_main() # Return to main menu by calling it out
                break
            if (input_1 == 'X'):
                print("*" * 31 + " THANK YOU " + "*" * 32 + "\n") # Handling Exit and print out thank you
                break
            if (input_1 == 'P'):
                print("\n" * 10)
            ...
            try:        #Cautions Error Handling to prevent program crashing and hand out exceptions as a readable error to notify user
                ...
                if ((int(input_1) <= len(list_foods) and int(input_1) > 0) or (int(input_1) <= len(list_drinks) + 40 and int(input_1) > 40)):
                     ...
                     if int(input_2) > 0:
                        ...
                        print("\n" * 10)
                        print("Your Order Has Placed Successfully! We Will Surve You With In 30 Min.")
                        print()
    ...
    while True:
        ...
        try:
            ...
            if (int(input_1) > 80) and (int(input_1) < 100):
                print("\n" * 10)
                print("Your Order Has Placed Successfully! " + str(list_services[int(input_1) - 81])) # Adding services to orders array (AND) encapsulate errors  with try/except
...
def def_report()    :
    while True:
        print("*" * 33 + "REPORT" + "*" * 33 + "\n")
        file_report = open(''+navigator_symbol+'/home/mukesh/AttainU/python-project-mukesh-maurya-au9/DeliceusGo/files/report.fsd', 'r').read() # Reading out reports from report.fsd
        print(file_report)
        print("\n(M) MAIN MENU          (X) EXIT\n" + "_" * 72)
...
def def_payment():
    while True:
        ...
        print("\n (P) PAY           (M) MAIN MENU           (R) REPORT          (X) EXIT\n" + "_" * 72)
        input_1 = str(input("Please Select Your Operation: ")).upper()
        if (input_1 == 'P'):
            print("\n" * 10)
            print("Your Payment has Successfully Paid!")
            print()
            file_report = open(''+navigator_symbol+'/home/mukesh/AttainU/python-project-mukesh-maurya-au9/DeliceusGo/files/report.fsd', 'a') # Save it into a file
            ...
            break
        elif ('X' in input_1) or ('x' in input_1):
            print("*" * 32 + " THANK YOU for Visiting on DeliceusGo! " + "*" * 31 + "\n")
            break
        else:
            print("\n" * 10 + "ERROR: Invalid Input (" + str(input_1) + "). Try again!")
